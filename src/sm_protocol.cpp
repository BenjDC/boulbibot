...
int sm_protocol::pingCommand(uint8_t id, uint16_t *model_number, uint8_t *firmware_version)
{
    ...
    // send packet
    MySerial->writePort(tx_packet_buffer, tx_packet_length);
    MySerial->clearPort();
...
// debug
    ...
    {
        ROS_INFO(tx_packet_buffer[index], HEX);
        ROS_INFO(" ");
    }
    ROS_INFO();
#endif
...
{
    while (MySerial->getBytesAvailable() > 0)
    {
        MySerial->readPort(rx_packet_buffer, RX_BUFFER_SIZE);
    }
}
...
bool sm_protocol::decodePacket(int inBytes)
{
    ...
    rx_packet_id = rx_packet_buffer[PKT_ID];
    ...
    rx_packet_crc = MAKE_SHORT(rx_packet_buffer[PKT_LENGTH_H + rx_packet_payload_length - 1], rx_packet_buffer[PKT_LENGTH_H + rx_packet_payload_length]);
...
unsigned long sm_protocol::micros()
{
    struct timeval tv;
